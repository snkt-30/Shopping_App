...
class ProductDetailView(View):
    def get(self, request, pk):
        ...
        item_alreadt_in_cart = False
        if request.user.is_authenticated:
            item_alreadt_in_cart = Cart.objects.filter(Q(product=product.id) & Q(user=request.user)).exists()
        return render(request, 'app/productdetail.html', {'product':product, 'item_alreadt_in_cart':item_alreadt_in_cart})
...
class CustomerRegistrationView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            messages.success(request, 'Congratulatiopns!! Registered Successfully')
            form.save()
        return render(request, 'app/customerregistration.html', {'form':form})
...
class ProfileView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            reg = Customer(user=usr, name=name, mobile_no=mobile_no, locality=locality, city=city, state=state, pincode=pincode)
            reg.save()
            messages.success(request, 'Congraculations!! Profile Updated Successfully')
        return render(request, 'app/profile.html', {'form':form, 'active':'btn-primary'})
